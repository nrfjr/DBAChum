...
onMounted(() => void mysqlStore.loadActivity(props.connectionId))
</script>
    ...
    <template v-else-if="activity">
      ...
      <template v-else>
        ...
        <ScrollableDataTable :empty="activity.items.length === 0" empty-message="No active MySQL/MariaDB statements." max-height="36rem">
          <template #header><tr><th>ID</th><th>User</th><th>Host</th><th>Database</th><th>Command</th><th>Elapsed</th><th>State</th><th>InnoDB transaction</th><th>Wait</th><th>Blocked by</th><th>SQL</th><th v-if="canOperate">Actions</th></tr></template>
          <tr v-for="item in activity.items" :key="item.connection_id">
            ...
            <td v-if="canOperate"><div class="database-inline-actions"><button type="button" class="secondary-button" :disabled="operations.busy" @click="runAction(item, 'cancel_query')">Cancel query</button><button type="button" class="danger-button" :disabled="operations.busy" @click="runAction(item, 'terminate')">Kill</button></div></td>
          </tr>
        </ScrollableDataTable>
      </template>
    </template>
