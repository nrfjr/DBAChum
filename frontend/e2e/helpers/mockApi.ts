...
export async function installMockApi(
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (path === '/provisioning/profiles' && method === 'POST') {
      const payload = request.postDataJSON() as Record<string, unknown>
      state.createdProvisioningProfiles.push(payload)
      return json(route, {
        id: 'profile-created',
        ...payload,
        ready: true,
    ...
    if (
      path === `/provisioning/oracle/${oracleConnection.id}/schemas`
      && method === 'GET'
    ) {
      return json(route, [{ name: 'ORMS' }])
    }

    if (
      path === `/provisioning/oracle/${oracleConnection.id}/schemas/ORMS/tables`
      && method === 'GET'
    ) {
      return json(route, [
        { owner: 'ORMS', name: 'USER_MASTER' },
      ])
    }
